...
##  Quick Start: Google Colab Training v2 with Live Web UI (8B Model on T4 GPU)

Train an **8B parameter model** using QLoRA in Google Colab (Tesla T4 15GB VRAM) with a real-time **Live Training Dashboard Web UI**:
...
# 2. Run automated Colab v2 training pipeline with Live Web UI Dashboard (Port 7860)
!python colab/run_colab_training_v2.py \
  --preset 8b \
  --max-steps 1000 \
  --micro-batch-size 2 \
  --grad-accum 16 \
...
### 🧠 Debug Brain Processor & Benchmark Suite

```bash
# Run step-by-step cognitive debug processor
python scripts/debug_aira_brain.py --prompt "Design dual-system architecture for Aira."

# Run custom benchmark suite for Aira models
python scripts/run_aira_benchmarks.py --model-tag "AiraPiX-8B-DualSystem-v1"
```
